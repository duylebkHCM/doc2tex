...
def get_rolling_crops(image, stride=[128, 128], window_size=512):
    ...
    for i in range(nwindows_vertical):
        for j in range(nwindows_horizontal):
            ...
            padded_window[
                y_center : y_center + window_height, x_center : x_center + window_width
            ] = rolling_window

            crops_list.append(rolling_window)
            padded_crops_list.append(padded_window)

            crops_info_list.append(
                (window_x_start, window_y_start, window_width, window_height)
            )
...
def postprocess(window_borders, scores, crops_info_list, window_size=512):
    ...
    for i in range(len(window_borders)):
        ...
        for k in range(len(window_border)):
            x0 = window_x_start + (
                window_border[k][0] - (window_size - window_width) // 2
            )
            y0 = window_y_start + (
                window_border[k][1] - (window_size - window_height) // 2
            )
            x1 = window_x_start + (
                window_border[k][2] - (window_size - window_width) // 2
            )
            y1 = window_y_start + (
                window_border[k][3] - (window_size - window_height) // 2
            )
