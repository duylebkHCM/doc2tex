...
def resize_gt(pdf_name="Alford94"):
    ...
    image_filelist = [
        file
        for file in os.listdir(os.path.join(image_dir, pdf_name))
    ...
    for image_filepath in image_filelist:
        ...
        resized_image = cv2.imread(
            os.path.join(resized_image_dir, pdf_name, image_filepath)
        )
        ...
        final_height_ratio = (
            final_height * intermediate_height_ratio / intermediate_height
        )
        ...
        cv2.imwrite(
            os.path.join(output_image_dir, pdf_name, str(page_id) + ".png"), final_image
        )

        if math_file_present:
            out_math_file = os.path.join(
                output_math_dir, pdf_name, str(page_id) + ".pmath"
            )
        ...
        if char_file_present:
            out_char_file = os.path.join(
                output_char_dir, pdf_name, str(page_id) + ".pchar"
            )
