...
def character_level_score(args):
    ...
    for char_info in char_bbs:
        ...
        for current_math_bb in det_math_bbs:
            math_bb = [
                float(current_math_bb[1]),
                float(current_math_bb[2]),
                float(current_math_bb[3]),
                float(current_math_bb[4]),
            ]
...
def box_level_granular_eval(
    ...
    for filename in training_pdf_names:
        ...
        for page in current_det[0]:
            ...
            for det in coarse:
                if (
                    gt_math_bbs[filename][str(int(float(page)))][int(det[3:]) - 1][5]
                    > 1
                ):
    ...
    # precision
    print(
        "Number of single character regions correctly detected IOU50, IOU75 ",
        single_char_det,
    )
    ...
    print(
        "Number of multi character regions correctly detected IOU50, IOU75 ",
        multi_char_det,
    )
    ...
    for pdf_name in training_pdf_names:
        for page in det_math_boxes[pdf_name]:
            for det in det_math_boxes[pdf_name][page]:
                ...
                det_bb = [float(det[1]), float(det[2]), float(det[3]), float(det[4])]

                if page not in gt_math_boxes[pdf_name]:
                    continue

                for gt in gt_math_boxes[pdf_name][page]:
                    gt_bb = [float(gt[1]), float(gt[2]), float(gt[3]), float(gt[4])]
        ...
        for page in all_math_boxes[pdf_name]:
            ...
            for char_info in char_bbs:
                for math_bb in math_boxes:
                    current_char_bb = [
                        float(char_info[1]),
                        float(char_info[2]),  # TODO index from 1
                        float(char_info[3]),
                        float(char_info[4]),
                    ]
...
if __name__ == "__main__":
    ...
    test_gt_math_dir = sys.argv[
        5
    ]  # /home/psm2208/Workspace/Task3_Detection/Test/test_math/
    ...
    char_level_eval(
        training_pdf_names,
        total_math_char,
        copy.deepcopy(gt_math_bbs),
        copy.deepcopy(det_math_bbs),
        copy.deepcopy(char_bbs),
    )

    box_level_granular_eval(
        training_pdf_names,
        total_math_char,
        gt_math_dir,
        detected_math_dir,
    ...
    find_merged_regions(
        training_pdf_names, copy.deepcopy(gt_math_bbs), copy.deepcopy(det_math_bbs)
    )
