        ...
        for i, det_math1 in enumerate(det_page_math):
            ...
            for j, det_math in enumerate(det_page_math):
                if i != j:
                    ...
                    )  # feature_extractor.vertical_dist_bb(det_page_math[i], det_page_math[j])
            ...
            if len(inside_gt_dict[i].intersection(inside_gt_dict[min_idx])) > 0:
                ...
                segmentation_gt.append(
                    feature_extractor.extract_features(
                        det_page_math[i], det_page_math[min_idx], 1
                    )
                )
            else:
                # negative example
                segmentation_gt.append(
                    feature_extractor.extract_features(
                        det_page_math[i], det_page_math[min_idx], 0
                    )
                )
    ...
    for pdf_name in pdf_names:
        ...
        if pdf_name != "":
            ...
            for page_num in pages:
                gt_page_math = gt_math_regions[
                    np.where(gt_math_regions[:, 0] == page_num)
                ]
                gt_page_math = gt_page_math[:, 1:]

                det_page_math = det_math_regions[
                    np.where(det_math_regions[:, 0] == page_num)
                ]
                det_page_math = det_page_math[:, 1:]

                pages_list.append(
                    [output_dir, pdf_name, page_num, gt_page_math, det_page_math]
                )
