...
def combine_math_regions(args):
    ...
    try:
        ...
        if if_visualize == 1:
            visualize.draw_all_boxes(
                image, processed_math_regions, math_regions, gt_regions, output_image
            )
        ...
        print(
            "Exception while processing ", pdf_name, " ", page_num, " ", sys.exc_info()
        )
...
def label_regions(math_regions, image):
    ...
    for label, math_region in enumerate(math_regions):
        labeled[
            math_region[1] : math_region[3], math_region[0] : math_region[2]
        ] = label
...
def char_algo(math_regions, char_data, image, algorithm="equal", thresh_votes=20):
    ...
    for char_box in char_data:
        ...
        if (
            np.count_nonzero(
                votes[
                    int(char_box[1]) : int(char_box[3]),
                    int(char_box[0]) : int(char_box[2]),
                ]
            )
            > 100
        ):
...
def clustering(math_regions, char_data, image, algorithm, thresh_votes):
    ...
    for math_region in math_regions:
        center = [
            (math_region[0] + math_region[2]) / 2,
            (math_region[1] + math_region[3]) / 2,
        ]
...
def find_blank_rows(image, line_spacing=1):
    ...
    sizes = np.array([[bb.stop - bb.start for bb in line_bb] for line_bb in line_bbs])
    ...
    for pdf_name in pdf_names:
        ...
        if pdf_name != "":
            ...
            for root, dirs, _ in os.walk(
                os.path.join(annotations_dir, pdf_name), topdown=False
            ):
                for dir in dirs:
                    for filename in os.listdir(
                        os.path.join(annotations_dir, pdf_name, dir)
                    ):
                        if filename.endswith(".csv") or filename.endswith(".pmath"):
                            patch_num = os.path.splitext(filename)[0]
                            page_num = os.path.basename(
                                os.path.join(annotations_dir, pdf_name, dir)
                            )
            ...
            for filename in os.listdir(os.path.join(char_gt, pdf_name)):
                if filename.endswith(".csv") or filename.endswith(".pchar"):
                    ...
                    char_annotations_map[pdf_name][page_num] = os.path.join(
                        char_gt, pdf_name, filename
                    )
...
def stitch_patches(
    ...
    pages_list, annotations_map, char_annotations_map = read_page_info(
        filename, annotations_dir, image_dir, gt_dir, char_gt
    )
    ...
    for i, _ in enumerate(pool.imap_unordered(combine_math_regions, pooling_list), 1):
        ...
        if i % 100 == 0:
            current = time.time()
            print(
                "\nTime taken for last 100, total time:",
                current - start,
                current - init,
            )
            start = time.time()
