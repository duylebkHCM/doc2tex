...
def create_bb(math_regions_chars, char_info):
    ...
    for region in math_regions_chars:
        ...
        for char_id in region:
            if len(box) == 0:
                box = [
                    float(char_info[char_id][0]),
                    float(char_info[char_id][1]),
                    float(char_info[char_id][2]),
                    float(char_info[char_id][3]),
                ]
            else:
...
def contract(im_bw, box):
    ...
    right = (
        box[0] + len(cols_with_pixels) - np.argmax(cols_with_pixels[::-1] == True) - 1
    )
    bottom = (
        box[1] + len(rows_with_pixels) - np.argmax(rows_with_pixels[::-1] == True) - 1
    )
...
def create_gt_math(
    filename, image_dir, char_dir, output_dir="/home/psm2208/data/GTDB/annotationsV2/"
):
                    ...
                    if name.endswith(".pchar"):
                        ...
                        pages_list.append(
                            (
                                ...
                                int(page_num),
                                os.path.join(output_dir, pdf_name + ".csv"),
                            )
                        )
                        # page_num + ".pmath")))
    pdf_names.close()
