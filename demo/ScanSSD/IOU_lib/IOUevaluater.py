...
def create_doc_bboxes_map(dir_path, flag):
    ...
    for filename in os.listdir(dir_path):
        ...
        # print(full_filepath)
        if (full_filepath.startswith(".")) or (
            not (full_filepath.endswith(".csv") or full_filepath.endswith(".math"))
        ):
            continue
        bboxes_map = {}
...
def fix_preds(input_dict, keyPairs, thre):
    ...
    for pair in validPairs:
        ...
        if pair[0] not in list(input_dict.keys()) or pair[1] not in list(
            input_dict.keys()
        ):
            continue
    ...
    for page_num in gt_page_bboxes_map:
        ...
        if outdir:
            out_file = open(
                os.path.join(
                    outdir,
                    pdf_name.split(".csv")[0] + "_" + str(page_num) + "_eval.txt",
                ),
                "w",
            )
            out_file.write(
            ...
            for gt_box in gt_boxes:
                ...
                for i in range(len(ious) - 1):
                    out_file.write(
                        "("
                        + str(round(ious[i][0], 2))
                        + " "
                        + str(ious[i][2].getImageName())
                        + "),"
                    )
                out_file.write(
                    "("
                    + str(round(ious[-1][0], 2))
                    + " "
                    + str(ious[-1][2].getImageName())
                    + ")\n"
                )
            out_file.close()
...
def write_html(gtFile, resultsFile, info, scores, destFile):
    destFile.write("<html>")
    destFile.write(
        '<head><link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css"><link href="/static/css/bootstrap.min.css" rel="stylesheet"></head>'
    )
    destFile.write("<body>")
    # writeCSS(destFile)
    destFile.write(
        "<blockquote><b>CROHME 2019</b> <h1> Formula Detection Results ( TASK 3 )</h1><hr>"
    )
    destFile.write(
        "<b>Submitted Files</b><ul><li><b>Output:</b> "
        + ntpath.basename(resultsFile)
        + "</li>"
    )
    ...
    destFile.write(
        "<p><b> Number of ground truth bounding boxes: </b>"
        + str(info["allGTbox"])
        + "<br /><b> Number of detected bounding boxes: </b>"
        + str(info["allDet"])
    )
    destFile.write("<hr>")
    # coarse results
    ...
    destFile.write(
        "<b>"
        + str(info["correctDet_c"])
        + "</b> Number of correctly detected bounding boxes</p>"
    )
    destFile.write("<hr>")
    # fine results
...
def pre_rec_calculate(count):
    if count["allDet"] == 0:
        ...
        scores = {
            "fine_f": round(f_f, 4),
            "fine_pre": round(pre_f, 4),
            "fine_rec": round(recall_f, 4),
            "coarse_f": round(f_c, 4),
            "coarse_pre": round(pre_c, 4),
            "coarse_rec": round(recall_c, 4),
        }
    return scores
...
def IOUeval(ground_truth, detections, outdir=None):  # ,
    ...
    for pdf_name in pdf_calcs:
        print(
            pdf_name,
            "\t",
            pdf_calcs[pdf_name]["coarse_f"],
            "\t",
            pdf_calcs[pdf_name]["fine_f"],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detections", type=str, required=True, help="detections file path"
    )
    parser.add_argument(
        "--ground_truth", type=str, required=True, help="ground_truth file path"
    )
