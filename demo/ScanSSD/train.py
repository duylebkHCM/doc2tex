...
def train(args):
    ...
    if not args.resume:
        ...
        torch.save(
            ssd_net.state_dict(),
            os.path.join(
                "weights_" + args.exp_name,
                "initial_" + str(args.model_type) + args.dataset + ".pth",
            ),
        )
    ...
    if args.visdom:
        ...
        iter_plot = create_vis_plot(
            "Iteration", "Loss", viz, "Training " + vis_title, vis_legend
        )
        epoch_plot = create_vis_plot(
            "Epoch", "Loss", viz, "Training " + vis_title, vis_legend
        )
    ...
    for iteration in range(args.start_iter, cfg["max_iter"]):
        ...
        loss = (
            args.alpha * loss_l + loss_c
        )  # TODO. For now alpha should be 1. While plotting alpha is assumed to be 1
        loss.backward()
        optimizer.step()
        ...
        if iteration != 0 and (iteration % epoch_size == 0):
            ...
            update_vis_plot(
                epoch, loc_loss, viz, conf_loss, epoch_plot, None, "append", epoch_size
            )
            ...
            if args.validation_data != "":
                ...
                if epoch == 1:
                    validation_plot = create_validation_plot(
                        ...
                        "Loss",
                        viz,
                        "Validating " + vis_title,
                        ["Validation"],
                    )
                else:
                    update_validation_plot(
                        epoch, validation_loss, viz, validation_plot, "append"
                    )
    ...
    logging.debug(
        "Final weights are saved at " + args.exp_name + "" + args.dataset + ".pth"
    )
...
def validate(args, net, criterion, cfg):
    ...
    try:
        ...
        logging.debug(
            "Time taken for validation " + str(datetime.timedelta(seconds=end - start))
        )
...
def create_validation_plot(
    epoch, validation_loss, _xlabel, _ylabel, viz, _title, _legend
):
    return viz.line(
...
def update_validation_plot(epoch, validation_loss, viz, window, update_type):
    viz.line(
        X=torch.ones((1, 1)).cpu() * epoch,
        Y=torch.Tensor([validation_loss]).unsqueeze(0).cpu(),
        win=window,
...
def update_vis_plot(
    iteration, loc, viz, conf, window1, window2, update_type, epoch_size=1
):
    viz.line(
...
def init_args():
    ...
    parser.add_argument(
        "--dataset", default="GTDB", choices=["GTDB"], type=str, help="choose GTDB"
    )
    parser.add_argument(
        "--dataset_root", default=GTDB_ROOT, help="Dataset root directory path"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--alpha", default=1.0, type=float, help="Alpha for the multibox loss"
    )
    parser.add_argument("--gamma", default=0.1, type=float, help="Gamma update for SGD")
    parser.add_argument(
        "--visdom", default=False, type=bool, help="Use visdom for loss visualization"
    )
    parser.add_argument(
        "--exp_name",
        default="math_detector",  # changed to exp_name from --save_folder
