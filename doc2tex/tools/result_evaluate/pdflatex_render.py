...
def process_args(args):
    parser = argparse.ArgumentParser(
        description="Render latex formulas for comparison. Note that we need to render both the predicted results, and the original formulas, since we need to make sure the same environment of rendering is used."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            'Output directory to put the rendered images. A subfolder with name "images_gold" will be created for the rendered gold images, and a subfolder with name "images_pred" will be created for the rendered predictions.'
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Replace flag, if set to false, will ignore the already existing images."
        ),
    )
...
def main(args):
        ...
        for line in csv_reader:
            ...
            lines.append(
                (
                    img_path,
                    label_gold,
                    os.path.join(gold_dir, img_path),
                    parameters.replace,
                )
            )
            lines.append(
                (
                    img_path,
                    label_pred,
                    os.path.join(pred_dir, img_path),
                    parameters.replace,
                )
            )
    ...
    _ = pool.map(main_parallel, lines)
    pool.close()
    pool.join()
...
def main_parallel(line):
    ...
    pre_name = os.path.basename(output_path).replace("/", "_").replace(".", "_")
    l = l.strip()
    ...
    if replace or (not os.path.exists(output_path)):
        ...
        if not os.path.exists(pdf_filename):
            ...
            os.system(
                "convert -density 200 -quality 100 -define colorspace:auto-grayscale=false -define png:color-type=6 %s %s"
