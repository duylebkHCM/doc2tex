...
import time
from tools.build_data.latex_processing.extract_latex import remove_labels
...
def parse_command_line():
    ...
    parser.add_argument(
        "--output", "-o", default=None, help="TeX output file with \\newcommand"
    )
...
def sweep(t, cmds):
    ...
    for c in cmds:
        ...
        if nargs == 0:
            ...
            if num_matches > 0:
                t = re.sub(
                    r"\\%s([\W_^\dĊ])" % c[0], r"%s\1" % c[-1].replace("\\", r"\\"), t
                )
        else:
            matches = re.findall(
                r"(\\%s(?:\[(.+?)\])?" % c[0]
                + r"{(.+?)}" * (nargs - (1 if optional else 0))
                + r")",
                t,
            )
...
def unfold(t):
    ...
    for i in reversed(range(len(cmds))):
        if cmd_names[cmds[i][0]] > 1:
            ...
            logging.debug(
                "Command recognition pattern didn't work properly. %s"
                % (undo_bracket_replace(cmds[i][-1]))
            )
            del cmds[i]
    start = time.time()
    try:
        for i in range(10):
            ...
            if (
                time.time() - start > 5
            ):  # not optimal. more sophisticated methods didnt work or are slow
                raise TimeoutError
