...
from doc2tex.utils.model_utils import adjust_learning_rate, update_summary, save_checkpoint
from doc2tex.utils.general_utils import cal_elasped_time
from doc2tex.modules.build_model import Model
from doc2tex.modules.converter import create_converter
from doc2tex.modules.loss import create_criterion, criterion_kwargs
from doc2tex.modules.optim import create_optimizer, optimizer_kwargs
from doc2tex.engine.inferencing import validation_step
...
def validation(
    ...
    exec_time = cal_elasped_time(exec_time)
    total_time = cal_elasped_time(total_time)
    loss_log = f'[{iteration+1}/{config["num_iter"]}] Train loss: {train_loss_avg.val():0.5f}, Valid loss: {valid_loss:0.5f}, Learning rate: {cur_lr:0.5f}, Val_elapsed_time: {exec_time}, Total_elapsed_time: {total_time}'
    ...
    for cost, gt, pred, img_name in sorted_res[:10]:
        predicted_result_log += f"Loss:{cost:0.4f}\nName:{img_name:25s}\nGround Truth:{gt:25s}\nPrediction{pred:25s}\nT/F:{str(pred == gt)}\n"
